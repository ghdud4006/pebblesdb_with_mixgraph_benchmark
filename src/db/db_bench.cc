...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (sscanf(argv[i], "--keyrange_num=%"PRId64"%c", &n64, &junk) == 1) {
    ...
    } else if (sscanf(argv[i], "--mix_max_scan_len=%"PRId64"%c", &n64, &junk) == 1) {
      FLAGS_mix_max_scan_len = n64;
    } else if (sscanf(argv[i], "--mix_ave_kv_size=%"PRId64"%c", &n64, &junk) == 1) {
      FLAGS_mix_ave_kv_size = n64;
    } else if (sscanf(argv[i], "--mix_max_value_size=%"PRId64"%c", &n64, &junk) == 1) {
